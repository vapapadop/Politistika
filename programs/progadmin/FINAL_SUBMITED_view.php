...
$layout->blocks["top"][] = "details";$page_layouts["FINAL_SUBMITED_view"] = $layout;
...
if (!sizeof($data)) {
	header("Location: FINAL_SUBMITED_list.php?a=return");
	exit();
}
...
$arr = array();
$arr['fName'] = "sch_id1";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("sch_id1");
$fieldsArr[] = $arr;
...
//sch_id1 - 
	...
	$value = $pageObject->showDBValue("sch_id1", $data, $keylink);
	if($mainTableOwnerID=="sch_id1")
		$ownerIdValue=$value;
	$xt->assign("sch_id1_value",$value);
	if(!$pageObject->isAppearOnTabs("sch_id1"))
		$xt->assign("sch_id1_fieldblock",true);
	else
		$xt->assign("sch_id1_tabfieldblock",true);
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "FINAL SUBMITED";
		$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='FINAL_SUBMITED_edit.php?".$editlink."'\"");
