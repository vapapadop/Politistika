...
function runner_mail_smtp($params)
{
	...
	if ("1" == "1" && "vapapado"!="" || isset($params['username']))
	{
		$mail->SMTPAuth   = true;                  				// enable SMTP authentication
		$mail->Username   = isset($params['username']) ? $params['username'] : "vapapado";  		// SMTP username
		$mail->Password   = isset($params['password']) ? $params['password'] : "";   // SMTP password
	}else{
		$mail->SMTPAuth = false;
	}
	if("1" == "1" && "mail.att.sch.gr" != "" || isset($params['host']))
	{
		$mail->Host = isset($params['host']) ? $params['host'] : "mail.att.sch.gr";     // sets SMTP server
	}
