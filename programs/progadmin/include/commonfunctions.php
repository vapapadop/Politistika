...
function checkTableName($shortTName, $type=false)
{
	...
	if ("ft_form_4" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pschools" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("settings" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("FINAL_SUBMITED" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("FINAL_STOPED" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("prosorines" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	return false;
}
...
function GetUserPermissionsStatic($table="")
{
	...
	if($table=="ft_form_4" && $sUserGroup=="admin")
	{
			return "AEDSPI";
	}
//	default permissions	
	...
	if($table=="pschools" && $sUserGroup=="admin")
	{
			return "AEDSPI";
	}
//	default permissions	
	...
	if($table=="settings" && $sUserGroup=="admin")
	{
			return "AEDSPI";
	}
//	default permissions	
	...
	if($table=="FINAL SUBMITED" && $sUserGroup=="admin")
	{
			return "AEDSPI";
	}
//	default permissions	
	if($table=="FINAL SUBMITED")
	{
	return "SP";
	}
	if($table=="FINAL_STOPED" && $sUserGroup=="admin")
	{
			return "AEDSPI";
	}
//	default permissions	
	if($table=="FINAL_STOPED")
	{
	return "EDSP";
	}
	if($table=="prosorines" && $sUserGroup=="admin")
	{
			return "AEDSPI";
	}
//	default permissions	
...
function inFacebook(){
	...
	if ($signed_request) {
		...
		$strSQL = "select * from ".AddTableWrappers("pschools")." where ".AddFieldWrappers(GetUserNameField())."='".$strUsername."'";
...
function AfterFBLogIn($pUsername, $pPassword){
	...
	$strSQL = "select * from ".AddTableWrappers("pschools")." where ".AddFieldWrappers($cUserNameField)."=".$strUsername."";
	$rs = db_query($strSQL,$conn);
 	$data = db_fetch_array($rs);
	if($data){
		$logged=true;
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password)
{
	global $globalEvents;
		...
		$_SESSION["_ft_form_4_OwnerID"] = $data["username"];
