...
$layout->blocks["top"][] = "details";$page_layouts["pschools_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pschools_inline_add.htm";
else
	$templatefile = "pschools_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pschools_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing sch_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sch_id = $pageObject->getControl("sch_id", $id);
		$control_sch_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sch_id - end
//	processing submited - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_submited = $pageObject->getControl("submited", $id);
		$control_submited->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing sch_code - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sch_code = $pageObject->getControl("sch_code", $id);
		$control_sch_code->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sch_code - end
//	processing sch_perioxi - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sch_perioxi = $pageObject->getControl("sch_perioxi", $id);
		$control_sch_perioxi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing sch_dimos - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sch_dimos = $pageObject->getControl("sch_dimos", $id);
		$control_sch_dimos->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sch_dimos - end
//	processing sch_phone - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sch_phone = $pageObject->getControl("sch_phone", $id);
		$control_sch_phone->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pschools_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pschools_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pschools_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["sch_id"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["sch_id"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["sch_id"]=@$avalues["sch_id"];
	$defvalues["submited"]=@$avalues["submited"];
	$defvalues["username"]=@$avalues["username"];
	$defvalues["password"]=@$avalues["password"];
	$defvalues["sch_code"]=@$avalues["sch_code"];
	$defvalues["sch_perioxi"]=@$avalues["sch_perioxi"];
	$defvalues["sch_name"]=@$avalues["sch_name"];
	$defvalues["sch_dimos"]=@$avalues["sch_dimos"];
	$defvalues["sch_phone"]=@$avalues["sch_phone"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("sch_id_label","<label for=\"".GetInputElementId("sch_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("submited_label","<label for=\"".GetInputElementId("submited", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sch_code_label","<label for=\"".GetInputElementId("sch_code", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sch_perioxi_label","<label for=\"".GetInputElementId("sch_perioxi", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sch_dimos_label","<label for=\"".GetInputElementId("sch_dimos", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sch_phone_label","<label for=\"".GetInputElementId("sch_phone", $id, PAGE_ADD)."\">","</label>");
