...
$layout->blocks["top"][] = "details";$page_layouts["prosorines_view"] = $layout;
...
if (!sizeof($data)) {
	header("Location: prosorines_list.php?a=return");
	exit();
}
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "prosorines";
		$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='prosorines_edit.php?".$editlink."'\"");
