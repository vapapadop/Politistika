...
{	
	...
	*/
	...
	{	
		...
		//for display name 
		$displayuserheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby="."Asch_name\"";
		...
		if ($this->sortByGroup === false && $this->sortField != ""){
			if ($this->sortField == "username"){
			...
			else if ($this->sortField == "sch_name"){
				...
				if($this->sortOrder == "A") {
					...
					$displayuserheaderlink_attrs.="Dsch_name";
				} else {
					if($this->is508)
					...
					$displayuserheaderlink_attrs.="Asch_name";
				}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			$row["displayusername"] = htmlspecialchars($data["sch_name"]); //htmlspecialchars($data["username"]);
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "username" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="sch_name"){
