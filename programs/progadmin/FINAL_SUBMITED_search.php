...
$layout->blocks["top"][] = "search";$page_layouts["FINAL_SUBMITED_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "FINAL_SUBMITED_search.htm";
...
$params['shortTableName'] = 'FINAL_SUBMITED';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("sch_id1"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("sch_id1")] = GetTableURL($pageObject->pSet->getLookupTable("sch_id1"));
	
	$pageObject->fillFieldToolTips("sch_id1");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("sch_id1");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "sch_id1";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("sch_id1_label","<label for=\"".GetInputElementId("sch_id1", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("sch_id1_label", true);
	
	$xt->assign("sch_id1_fieldblock", true);
	$xt->assignbyref("sch_id1_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("sch_id1_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("sch_id1_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_sch_id1", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("sch_id1");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"sch_id1", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"sch_id1", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
